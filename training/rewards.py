...
def extract_designed_sequence(completion: str)-> Optional[bool]:
    ...
    if all(char in valid_aas for char in answer_text):
...
def reward_non_symmetric(x, xmin=0.0, xmax=1.0, threshold=0.5, alpha=1.3, beta=0.7):
    if x < threshold:
        # wrong → [0, 0.5]
        norm = (x - xmin) / (threshold - xmin)
        return 0.5 * (norm ** alpha)
    else:
        # correct → [0.5, 1]
        norm = (x - threshold) / (xmax - threshold)
        return 0.5 + 0.5 * (norm ** beta)
