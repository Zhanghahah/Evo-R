...
def load_mlp_classifier(mlp_model_path: str, embedding_file: str, obj_type):
    ...
    return mlp_model, embeddings_dict


def peptide_mlp_binary_infer(sample_data: dict) -> float:
    global mlp_model, embeddings_dict

    if mlp_model is None or embeddings_dict is None:
        raise RuntimeError("MLP model not loaded. Call load_mlp_classifier() first.")

    # Extract gene names from keywords (assuming format: "seq_A|seq_B")
    keywords = sample_data.get("keywords", "")
    if not keywords or "|" not in keywords:
        return 0.5  # Default probability if no valid amino sequences

    peptides_names = keywords.split("|")
    if len(peptides_names) < 2:
        return 0.5  # Need at least 2 sequences

    pep_sq1, pep_sq2 = peptides_names[0], peptides_names[1]
    sq1_plddt, sq2_plddt = sample_data["seqA_plddt"], sample_data["seqB_plddt"]
    annotated = sample_data["label"]

    # double-check the label is right
