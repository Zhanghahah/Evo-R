...
NUM_GENERATIONS = 4
SAVE_EVERY = 1000
OUTPUT_DIR = "./checkpoints/MCL1_train_sampled_200/"

# Global step counter
STEP_COUNT = 0

# MLP classifier configuration (mlp was not trained on k562 cells)
MLP_MODEL_PATH = "MCL1_mlp_classifier.pth"
EMBEDDING_FILE = "MCL1_peptide_esm_embedding.pkl"

# Dataset paths
DATASET_PATHS = [
    "MCL1_train_sampled_2000.csv"
]
...
def create_mlp_labeled_dataset_generator(dataset_df: pd.DataFrame, tokenizer, balance_pos_neg: bool = True):
    ...
    for i in range(dataset_length):
        ...
        sample_data = {
            ...
            "seqA_plddt": row["seqA_plddt"],
            "seqB_plddt": row["seqB_plddt"],
            "label": row["label"]
        }
...
def compute_simple_reward(
    ...
    **kwargs
    ...
    for completion, lbl, class_list, confidences, keyword_list in zip(
        ...
        scaled_answer_reward = reward_non_symmetric(answer_reward)

        # Combine rewards (answer reward gets 2x weight as it's most important)
        total_score = 0.5 * format_reward + 2.5 * scaled_answer_reward + 0.5 * mention_reward
        ...
        if STEP_COUNT % 50 == 0:
            ...
            logger.info(f"  Answer reward: {answer_reward:.3f}")
            logger.info(f"  Total score: {total_score:.3f}")
            logger.info("-"*80)

            # Print expected vs predicted
            logger.info(f"EXPECTED vs PREDICTED:")
            logger.info(f"  binarized label: {lbl}")
            logger.info(f"  Possible classes: {class_list}")
            logger.info(f"  Label confidences: {confidences}")
